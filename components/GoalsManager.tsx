...
const emptyForm = { name: '', target_amount: '', current_amount: '', deadline: '', color: '#6366f1' }
  ...
  return (
    <>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        {goals.length === 0 ? (
          <div className="col-span-2 rounded-2xl p-8 text-center" style={{ background: '#1a1a24', border: '1px solid #2a2a3a' }}>
            <p style={{ color: '#9090a8' }}>Нет целей</p>
          </div>
        ) : (
          goals.map(g => {
            ...
            return (
              <div key={g.id} className="rounded-2xl p-4" style={{ background: '#1a1a24', border: `1px solid ${g.color}30` }}>
                <div className="flex items-start justify-between mb-3">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      ...
                      {g.deadline && <p className="text-xs" style={{ color: '#9090a8' }}>До {formatDate(g.deadline)}</p>}
                    </div>
                  </div>
                  <div className="flex gap-1">
                    <button onClick={() => openEdit(g)} className="p-1.5 rounded-lg text-xs hover:bg-white/10" style={{ color: '#9090a8' }}>✎</button>
                    <button onClick={() => handleDelete(g.id)} className="p-1.5 rounded-lg text-xs hover:bg-red-500/20" style={{ color: '#ef4444' }}>✕</button>
                  </div>
                </div>
                <div className="mb-2">
                  <div className="flex justify-between text-xs mb-1.5" style={{ color: '#9090a8' }}>
                  ...
                  <p className="text-xs mt-1" style={{ color: done ? '#10b981' : '#9090a8' }}>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: '#000000aa' }}>
          <div className="w-full max-w-md rounded-2xl p-6" style={{ background: '#1a1a24', border: '1px solid #2a2a3a' }}>
            ...
            <div className="space-y-4">
              ...
              <div>
                <label className="text-xs font-medium mb-1.5 block" style={{ color: '#9090a8' }}>Целевая сумма ₽</label>
              ...
              <div>
                <label className="text-xs font-medium mb-1.5 block" style={{ color: '#9090a8' }}>Уже накоплено ₽</label>
              ...
              <div>
                <label className="text-xs font-medium mb-1.5 block" style={{ color: '#9090a8' }}>Дата дедлайна (необязательно)</label>
      ...
      {showDeposit && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: '#000000aa' }}>
          <div className="w-full max-w-sm rounded-2xl p-6" style={{ background: '#1a1a24', border: '1px solid #2a2a3a' }}>
            ...
            <p className="text-sm mb-5" style={{ color: '#9090a8' }}>{showDeposit.name}</p>
